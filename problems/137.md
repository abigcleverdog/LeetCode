...
my thoughts:
```
1. study bit manipulation.

2. partition the list till the num is found.

3. build a 32 element array, track each bit.
```
...
class Solution:
    def singleNumber(self, nums):
        ...
        two = 0
        for n in nums:
            # one^n stores n in one;  & ~two clear n from one if n is in two already
            one = (one^n) & ~two
            two = (two^n) & ~one
        return one
    ...
    def singleNumber(self, nums):
        ...
        for i in range(32):
            ...
            if i == 31 and rem:
				...
				# can be written as: res |= rem << i
                res += 2**i if rem else 0
            #print(n, res)
