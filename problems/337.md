...
class Solution:
    ...
    def r(self, root):
        if not root:
            return [0, 0]        
        l, r = self.r(root.left), self.r(root.right)
...
class Solution:
    ...
    def r(self, root):
        ...
        val = 0
        if root.left:
            val += (self.r(root.left.left) + self.r(root.left.right))
        if root.right:
            val += (self.r(root.right.left) + self.r(root.right.right))
        l, r = self.r(root.left), self.r(root.right)
        self.d[root] = max(root.val + val, l + r)
        return self.d[root]
```
